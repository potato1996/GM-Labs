...
)
{
  ...
  SparseMatrixXcd A(count, F.rows());
  A.setFromTriplets(t.begin(), t.end());
  SparseMatrixXcd Q = A.adjoint() * A; // Q: #Faces x #Faces

  // Get the free variable list
  const int num_freevar = F.rows() - hard_id.size();
  const int num_consvar = hard_id.size();

  VectorXi free_id(num_freevar);
  // Here we assuming that human cannot create too much hard constrains..
  // So this function should be efficient enough.
  auto is_cons_id = [&hard_id, &num_consvar](int c_id) {
    for (int i = 0; i < num_consvar; ++i)
    {
      if (hard_id(i) == c_id)
      {
        return true;
  ...
  for (int f_id = 0, face_id = 0; face_id < F.rows(); ++face_id)
  {
    if (!is_cons_id(face_id))
    {
      free_id(f_id) = face_id;
      f_id++;
    }
  }

  // Construct Qff and Qfc
  SparseMatrixXcd Qff(num_freevar, num_freevar);
  SparseMatrixXcd Qfc(num_freevar, num_consvar);
  igl::slice(Q, free_id, free_id, Qff);
  igl::slice(Q, free_id, hard_id, Qfc);

  // Compute Xc
  MatrixXcd Xc(num_consvar, 1);
  for (unsigned r = 0; r < num_consvar; ++r)
  {
    int f = hard_id(r);
  ...
  // Solve the linear system: QffXf + QfcXc = 0
  SimplicialLDLT<SparseMatrixXcd> solver;
  solver.compute(Qff);
  assert(solver.info() == Success);
  MatrixXcd Xf = solver.solve(-Qfc * Xc);
  assert(solver.info() == Success);

  // Convert the interpolated polyvector into Euclidean vectors
  MatrixXd R(F.rows(), 3);
  for (int r = 0; r < num_consvar; ++r)
  {
    int f = hard_id(r);
    R.row(f) = T1.row(f) * Xc(r).real() + T2.row(f) * Xc(r).imag();
  }
  for (int r = 0; r < num_freevar; ++r)
  {
    int f = free_id(r);
    R.row(f) = T1.row(f) * Xf(r).real() + T2.row(f) * Xf(r).imag();
  }
  return R;
}